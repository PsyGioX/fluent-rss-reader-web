...
**Диалоги**
- Системный `prompt()` (создание новой категории) заменён на модалку в общей системе «шитов» — текстовое поле, кнопки «Отмена»/«Сохранить», Enter подтверждает, закрытие крестиком/фоном/`Esc` равнозначно отмене. `alert()` и `confirm()` в коде не использовались, заменять было нечего.

**Исправлено после первого прод-деплоя**
- Критический баг: `/api/*` отвечали 500 на любой ленте. Причина — обработчики полагались на `req.query` и `res.status().send()`, удобства, которые Vercel добавляет к сырым `req`/`res` не всегда (ненадёжно именно для ESM-функций, а у проекта `"type": "module"`). Первое же обращение к `res.status()` бросало исключение вне перехвата, и платформа возвращала пустой 500. Переписано на API самого Node.js (`res.statusCode`, `res.end()`, разбор `req.url` через `new URL()`), которое работает независимо от того, обёрнуты объекты платформой или нет. Каждый обработчик дополнительно завёрнут в `withSafety()` — любое неучтённое исключение теперь превращается в JSON с сообщением об ошибке вместо непрозрачного краша.
- CSP блокировала собственный `<script type="application/ld+json">` со структурированными данными (`script-src 'self'` без `'unsafe-inline'`). Сборка теперь считает SHA-256 конкретного блока на каждой языковой странице и добавляет точный хеш в `script-src` этой же страницы — блокировка снята без ослабления политики.
- Добавлен `<meta name="mobile-web-app-capable">` рядом с устаревшим `apple-mobile-web-app-capable`.
- Убран `"trailingSlash": true` из `vercel.json` — он добавлял лишний редирект перед каждым обращением к `/api/*`.
- Фавиконки лент, для которых сервис Google не находит иконку (404), теперь заменяются нейтральным значком, а не остаются битой картинкой.
